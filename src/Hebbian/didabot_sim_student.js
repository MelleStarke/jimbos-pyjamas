...
function senseTouch() {
	  /*
		 * Touch sensor simulation based on ray casting. Called from sensor
		 * object, returns nothing, updates a new reading into this.value.
		 * 
		 * Idea: Cast a ray with a certain length from the sensor, and check via
		 * collision detection if objects intersect with the ray. To determine
	  ...
	  if (rayLength > this.maxVal) {
	    ...
	    // Touch is binary : either touch or no touch
	    rayLength = 1;
	  }
...
function learnWeights(robot, distNodesActivation, touchLSensorActivation, touchRSensorActivation, touchNodesActivation, nrTouchSensors) {
	...
	//Forgetting:
	const avgTouchNodeActivation = (touchNodesActivation["l"] + touchNodesActivation["r"]) / nrTouchSensors;
	var forgetting = {ll: 0.0, lr: 0.0, rl: 0.0, rr: 0.0};
	for (key in forgetting) {
		forgetting[key] = simInfo.forgettingRate * avgTouchNodeActivation * robot.info.weights[key];
	}
	
	//Updating weights:
	for (key in robot.info.weights) {
		robot.info.weights[key] += (learning[key] - forgetting[key]) / nrTouchSensors;
	}
};
